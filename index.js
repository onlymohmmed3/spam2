...
class ConfigManager {
  ...
  initConfig() {
    if (!fs.existsSync(this.configPath)) {
      ...
      console.log(chalk.yellow('📝 Config file created! Please update your settings.'));
      process.exit(0);
    }
...
class DiscordSuperBotPro {
  ...
  async initialize() {
    try {
      ...
      // Set up admin password if not exists
  ...
  async setupAdminPassword() {
    const currentPassword = this.config.get('security.adminPassword');
    
    if (!currentPassword) {
      const defaultPassword = 'admin123';
      const hashedPassword = await bcrypt.hash(defaultPassword, 10);
      this.config.set('security.adminPassword', hashedPassword);
      
      Logger.warn(`⚠️  Default admin password set: ${defaultPassword}`);
      Logger.warn('⚠️  Please change it immediately after first login!');
    }
  }
